...
import { X, User, Package, Calendar, Clock, MapPin, Phone, Mail, AlertCircle, CheckCircle, Search, Info, ShoppingBag } from 'lucide-react';
import { axiosInstance } from '../../api/axiosInstance';
import orderAdminApi from '../../api/orderAdminApi';

const BookOrderModal = ({ user, onClose, onSuccess }) => {
    ...
    const [contactInfo, setContactInfo] = useState({
        ...
        address: {
            ...
            state: user?.state || '',
            pincode: '',
            mobile: user?.mobileNumber || '' // Mobile for address
        }
    });

    const [pincodeStatus, setPincodeStatus] = useState({ checking: false, available: null, message: '' });
    ...
    const handleSubmit = async () => {
        ...
        try {
            const payload = {
                ...
                contactInfo: {
                    ...contactInfo,
                    mobile: contactInfo.address.mobile || contactInfo.mobile, // Ensure top-level mobile is set
                    address: {
                        ...contactInfo.address,
                        landmark: '' // backend expects this but we remove it from UI
                    }
                },
                appointment: {
                ...
                <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between bg-indigo-600 text-white rounded-t-2xl">
                    <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                            ...
                            <p className="text-indigo-100 text-xs ml-2 uppercase tracking-wider font-semibold">Step {step}: {step === 1 ? 'Select Products' : step === 2 ? 'Details & Address' : 'Slot & Confirm'}</p>
                        </div>
                    </div>
                                                ...
                                                <div className="relative">
                                                    ...
                                                    <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
                                                        {pincodeStatus.checking ? (
                                                            <div className="animate-spin h-5 w-5 border-2 border-indigo-500 border-t-transparent rounded-full font-black"></div>
                                            ...
                                            <div>
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => {
                                                        ...
                                                        setContactInfo({
                                                            ...contactInfo,
                                                            mobile: val, // Also update top-level mobile
